...
var isInsideTaskOrDecisionItem = function (state) {
    ...
    return prosemirror_utils_1.hasParentNodeOfType([decisionItem, taskItem])(state.selection);
};
function keymapPlugin(schema) {
    ...
    var keymaps = {
        Backspace: function (state, dispatch) {
            ...
            dispatch(commands_1.splitListAtSelection(tr, schema));
            return true;
        },
        Delete: function (state, dispatch) {
            ...
            if (!isAtEndOfList) {
                ...
                commands_1.splitListAtSelection(tr, schema);
            ...
            if (!commands_1.isSupportedSourceNode(schema, selection)) {
        ...
        Enter: function (state, dispatch) {
            ...
            if (nodeIsTaskOrDecisionItem) {
                if (!isEmpty) {
                    tr.split($from.pos, 1, [
                        { type: nodeType, attrs: { localId: adf_schema_1.uuid.generate() } },
                    ]);
                    dispatch(tr);
                    return true;
                }
